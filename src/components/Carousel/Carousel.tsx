...
const Control: React.FC<ControlProps> = ({
  onClick,
  roundControls = false,
  disabledRightControl = false,
  disabledLeftControl = false,
}) => {
  return (
    <>
      <ControlButton
        ...
        slot="left"
        round={roundControls}
        disabled={disabledLeftControl}
      />
      <ControlButton
        onClick={onClick}
        slot="right"
        round={roundControls}
        disabled={disabledRightControl}
      />
    </>
  );
};
...
const Indicators: React.FC<IndicatorsProps> = ({
  ...
  useEffect(() => {
    if (selectedIndex >= 0 && selectedIndex < total) {
      ...
      if (
        total > max &&
        selectedIndex + (mult > 0 ? 1 : 0) > middleNumber &&
        selectedIndex + middleNumber + (mult > 0 ? 1 : 0) < total
      ) {
...
const Carousel: React.FC<Props> = React.forwardRef<
  HTMLDivElement,
  PropsWithRef<Props>
>(
  (
    ...
    const [currentIndex, setCurrentIndex] = useState<number>(
      forceSlideTo < slidesLength ? forceSlideTo : 0,
    );
    ...
    const renderSlide = (slide: ReactElement, index: number) => {
      return <Slide ref={slidesRef.current[index]}>{slide}</Slide>;
    };
    ...
    return (
      <CarouselStyled ref={getComponentRef(carouselRef, ref)} height={height}>
        {withControls && !autoSlide && (
          <Control
            onClick={handleSlideTo}
            roundControls={roundControls}
            disabledRightControl={isControlDisabled("right")}
            disabledLeftControl={isControlDisabled("left")}
          />
        )}
